...
drx <- "T://data//nsw_climate_environment_layers//maxent_models"
...
names_files <- unique(dirname(all_layers)) %>% 
                str_remove(., "T://data//nsw_climate_environment_layers//maxent_models/") %>% 
                str_replace_all(., "/", "_") 
...
save(pred_CCCMA, file = "output/predictions/pred_CCCMA.RData")
stack_CCCMA <- stack(pred_CCCMA)
writeRaster(stack_CCCMA, filename="output/predictions/C.tif", bylayer=TRUE, 
            suffix = names_CCCMA)
                                  
pred_CSIRO <- lapply(rasters_CSIRO, function(x){raster::predict(x,maxent_tuning_baseline)})          
save(pred_CSIRO, file = "output/predictions/pred_CSIRO.RData")
stack_CSIRO <- stack(pred_CSIRO)
writeRaster(stack_CSIRO, filename="output/predictions/C.tif", bylayer=TRUE, 
            suffix = names_CSIRO)

pred_ECHAM <- lapply(rasters_ECHAM, function(x){raster::predict(x,maxent_tuning_baseline)})          
save(pred_ECHAM, file = "output/predictions/pred_ECHAM.RData")
stack_ECHAM <- stack(pred_ECHAM)
writeRaster(stack_ECHAM, filename="output/predictions/C.tif", bylayer=TRUE, 
            suffix = names_ECHAM)
